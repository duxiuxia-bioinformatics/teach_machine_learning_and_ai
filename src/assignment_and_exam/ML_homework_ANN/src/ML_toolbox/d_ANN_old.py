...
class MyANN:
    ...
    def get_cross_entropy_cost_one_sample(self, actual, predicted):
        cost_one_sample = - actual * np.log(predicted) - (1.0 - actual) * (np.log(1.0 - predicted))
        return cost_one_sample
    ...
    def do_back_propagation(self, actual_output, a, w):
        ...
        d_total_cost_wrt_a_superscript_2 = np.matmul(delta['layer_3'].transpose(), w['layer_2'][:, 1:])
        delta['layer_2'] = d_total_cost_wrt_a_superscript_2.transpose() * self.get_derivative_of_a_wrt_z(a['layer_2'])[1:]
    ...
    def fit(self, X, Y):
        ...
        for index_iter in np.arange(start=1, stop=self.num_of_iterations, step=1):
            ...
            for index_sample in range(self.num_of_samples):
                ...
                if self.bool_is_classification:
                    cur_cost = self.get_cross_entropy_cost_one_sample(actual=y_one_sample, predicted=h[0][0])
                else:
                    cur_cost = self.get_mean_squared_cost_one_sample(actual=y_one_sample, predicted=h)
    ...
    def predict(self, X):
        ...
        for i in range(X.shape[0]):
            ...
            if self.bool_is_classification:
                ...
                y_predicted_list.append(cur_label_predicted)
            else:
                y_predicted_list.append(cur_a_predicted)
